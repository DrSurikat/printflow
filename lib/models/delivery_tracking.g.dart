// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'delivery_tracking.dart';

// **************************************************************************
// TypeAdapterGenerator
// **************************************************************************

class DeliveryTrackingAdapter extends TypeAdapter<DeliveryTracking> {
  @override
  final int typeId = 10;

  @override
  DeliveryTracking read(BinaryReader reader) {
    final numOfFields = reader.readByte();
    final fields = <int, dynamic>{
      for (int i = 0; i < numOfFields; i++) reader.readByte(): reader.read(),
    };
    return DeliveryTracking(
      id: fields[0] as String,
      orderId: fields[1] as String,
      orderNumber: fields[2] as String,
      carrier: fields[3] as DeliveryCarrier,
      trackingNumber: fields[4] as String,
      lastStatus: fields[5] as String,
      lastUpdated: fields[6] as DateTime,
      history: (fields[7] as List?)?.cast<String>(),
    );
  }

  @override
  void write(BinaryWriter writer, DeliveryTracking obj) {
    writer
      ..writeByte(8)
      ..writeByte(0)
      ..write(obj.id)
      ..writeByte(1)
      ..write(obj.orderId)
      ..writeByte(2)
      ..write(obj.orderNumber)
      ..writeByte(3)
      ..write(obj.carrier)
      ..writeByte(4)
      ..write(obj.trackingNumber)
      ..writeByte(5)
      ..write(obj.lastStatus)
      ..writeByte(6)
      ..write(obj.lastUpdated)
      ..writeByte(7)
      ..write(obj.history);
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is DeliveryTrackingAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}

class DeliveryCarrierAdapter extends TypeAdapter<DeliveryCarrier> {
  @override
  final int typeId = 9;

  @override
  DeliveryCarrier read(BinaryReader reader) {
    switch (reader.readByte()) {
      case 0:
        return DeliveryCarrier.novaPoshta;
      case 1:
        return DeliveryCarrier.ukrposhta;
      default:
        return DeliveryCarrier.novaPoshta;
    }
  }

  @override
  void write(BinaryWriter writer, DeliveryCarrier obj) {
    switch (obj) {
      case DeliveryCarrier.novaPoshta:
        writer.writeByte(0);
        break;
      case DeliveryCarrier.ukrposhta:
        writer.writeByte(1);
        break;
    }
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is DeliveryCarrierAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}
